...
def magnet(im,
    ...
    mode : str
        The mode used by numpy's pad function. Options are either `reflect` or
        `edge`. If reflect is used disconnected solids on the surface of the
        image are trimmed prior to padding.  The default mode is `reflect`.
    ...
    # get the skeleton
    if sk is None:
        sk = skeleton(im, padding, parallel, mode, **kwargs)
    else:
        if im.ndim == 3:
            _check_skeleton_health(sk)  # FIXME: Ensure sk is boolean
...
def insert_pore_bodies(sk, dt, pt, l_max=7):
    r"""
    ...
    Ps2 = ((pts + mx) > 0) * Ps  # Be careful, as not all merged points could get labelled!! Seems rare in 2D
    # results object
    fbd = Results()
    fbd.Ps = Ps
    fbd.Ps2 = Ps2
...
def spheres_to_network(sk, dt, fbd, pt, voxel_size=1, boundary_width=3):
    ...
    # pore volume
    _, count = np.unique(fbd.Ps, return_counts=True)
    p_volume = count[1:]
    # equivalent pore radius, assuming spheres for 3D and circles for 2D
    if ND == 2:
        p_radius = (p_volume/np.pi)**(1/2)
    else:
        p_radius = (3/4*p_volume/np.pi)**(1/3)
    ...
    net['pore.volume'] = p_volume * voxel_size**ND
    bw = boundary_width
...
def skeleton(im, padding=20, parallel=False, mode='edge', **kwargs):
    r"""
    ...
    skeleton by removing any floating solids in the original image.
    ...
    """
    if im.ndim == 3:  # trim floating solid from 3D images
        im = trim_floating_solid(im, conn=6, surface=False)
    # add pading
...
if __name__ == "__main__":
    ...
    equivalent = True
    twod = False
    names = ['Berea', 'A1', 'C1', 'C2', 'S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'S8', 'S9']
    # names=['Berea']
    K = np.zeros((len(names), 4))
    for i, name in enumerate(names):
        ...
        if not twod:
            # shape = [400, 400, 400]
            # path = f'C: \Users\mmcka\OneDrive - University of Waterloo\UW files\'
            # Projects\MAGNET\Images\Blunt
            ...
            # Parameters
            x, y, z = 5.345e-6, 5.345e-6, 5.345e-6
            Pin = 22.5
            Pout= 0
            A = (y * im.shape[1]) * (z * im.shape[2])
            L = x * im.shape[0]
            voxel_size = x
    
        if equivalent:
            sk = skeleton(im, padding=0, mode='reflect')
            pt = analyze_skeleton(sk)
            b = square(3) if im.ndim == 2 else cube(3)
            dt = spim.maximum_filter(edt(im), footprint=b)
            fbd = insert_pore_bodies(sk, dt, pt, l_max=7)
            net = spheres_to_network(sk, dt, fbd, pt, voxel_size=voxel_size, boundary_width=5)
    
        if not equivalent:
            net, sk, pt, fbd = ps.filters.magnet(im,
                                                 padding=40,
                                                 l_max=7,
                                                 mode='edge',
        ...
        if twod:
            ...
            plt.figure(2)
            plt.imshow(1/im + sk + pt.endpts*4 + pt.juncs*2)
            plt.axis('off')
        
            plt.figure(3)
            plt.imshow(np.flip((1/im + sk + (fbd.Ps>0)*2 + pt.endpts*8 + pt.juncs*4).T, axis=0))
            plt.axis('off')
        ...
        Ps_trim = np.append(h['disconnected_pores'], h['isolated_pores'])
        op.topotools.trim(net, pores=Ps_trim)
        ...
        Kavg = np.average([K_x, K_y, K_z])
        print(f'Kavg is: {Kavg/0.98e-12*1000:.2f} mD')
        
        K[i, :] = np.array([K_x, K_y, K_z, Kavg])

    np.savetxt('permeability-no-endpts.csv', K, delimiter=',')
