...
im_pores, im_throats = ps.networks.partition_skeleton(sk, juncs, dt)

new_pores, im_pores, new_throats = \
    ps.networks.find_throat_junctions(im, pores=im_pores, throats=im_throats, dt=dt)

# %% insert pores at junction points
pts = (im_pores + new_pores) > 0
fbd = ps.networks.insert_pore_bodies(sk, dt, pts)
# convert spheres to network dictionary
net = ps.networks.spheres_to_network(sk, dt, fbd, pts, res, boundary_width=0)
...
print('The expected permeability is 1800 mD')
