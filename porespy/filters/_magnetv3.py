...
if __name__ == "__main__":
    ...
    sk = skeleton(im, padding=40, mode='edge')
    pt = analyze_skeleton(sk)
    b = square(3)
    dt = spim.maximum_filter(edt(im), footprint=b)
    ...
    throats, num_throats = spim.label(throats, structure=s)
    slicess = spim.find_objects(throats)  # Nt by 2
    ss = extend_slice(slicess[i], im.shape, pad=np.max(dt))
    throats_im = throats[ss] == i+1
    plt.figure(2)
    plt.imshow(1/im[ss]+throats_im)
    plt.axis('off')
    # find minimum on throat
    throat_dt = dt[ss]*throats_im
    mn = np.where(throat_dt == np.min(throat_dt[throats_im != 0]))
    # we can get more than one min, that is why filtering dt would help
    c = [m[0] for m in mn]
